...
public class FirstFilter implements Filter {
	...
	public void destroy() {
		...
		// 필터 인스턴스 소멸 시 호출되는 메소드
		System.out.println("Filter destroy() 메소드 호출");
	
	}

	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		// Servlet 으로 request 가 전달되기 전에 요청을 가로채는 역할을 하는 메소드로, doFilter() 호출 전에 filter 에서 처리할 코드 작성
		System.out.println("Filter doFilter() 메소드 호출 1");
		
		// filter 처리 이후 다음 필터 또는 Servlet 의 doGet/doPost 호출
		chain.doFilter(request, response);
		
		// Servlet 처리 후 수행할 filter 코드 작성
		System.out.println("Filter doFilter() 메소드 호출 2");
	}
