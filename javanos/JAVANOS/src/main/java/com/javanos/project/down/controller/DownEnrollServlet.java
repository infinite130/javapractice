...
public class DownEnrollServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 
		 * DownService downService = new DownService(); int totalCount =
		 * downService.selectTotalCount(searchMap);
		 * 
		 * System.out.println("totaldownCount : " + totalCount);
		 * 
		 * 한 페이지에 보여 줄 게시물 수 int limit = 10; 한 번에 보여질 페이징 버튼의 갯수 int buttonAmount = 5;
		 * 
		 * 페이징 처리를 위한 로직 호출 후 페이징 처리에 관한 정보를 담고 있는 인스턴스를 반환받는다. SelectCriteria
		 * selectCriteria = null;
		 * 
		 * if(searchCondition != null && !"".equals(searchCondition)) { selectCriteria =
		 * Pagenation.getSelectCriteria(pageNo, totalCount, limit, buttonAmount,
		 * searchCondition, searchValue); } else { selectCriteria =
		 * Pagenation.getSelectCriteria(pageNo, totalCount, limit, buttonAmount); }
		 * 
		 * System.out.println(selectCriteria);
		 * 
		 * 조회해온다 List<DownDTO> downList = downService.selectDownList(selectCriteria);
		 * 
		 * System.out.println("boardList : " + downList);
		 * 
		 * String path = ""; if(downList != null) { path =
		 * "/WEB-INF/views/down/DownEnrollForm.jsp"; request.setAttribute("downList",
		 * downList); request.setAttribute("selectCriteria", selectCriteria); }
		 */
		/*
		 * else { path = "/WEB-INF/views/common/failed.jsp";
		 * request.setAttribute("message", "게시물 목록 조회 실패!"); }
		 */

		/*
		 * List<DownDTO> downList = new DownService().selectAllDownList();
		 * System.out.println(downList);
		 * 
		 * request.setAttribute("name", downList);
		 */
