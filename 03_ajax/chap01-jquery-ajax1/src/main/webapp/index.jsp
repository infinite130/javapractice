...
<body>
	...
	$("#send-no-data").click(function(){
		...
		$.ajax({					// -> 중괄호로 열고 닫음 + key:value 로 프로퍼티 구성 + 프로퍼티 간,
			url: "/chap01/receive",				// 요청 보낼 url 
			type: "get",				// 요청 방식
			success: function(data) {	// 성공 시 콜백 함수(매개변수로 성공 데이터 받아 쓸 수 있음)
				alert(data);
			},
			error: function(error) {	// 실패 시 콜백 함수(매개변수로 에러 정보 받아 쓸 수 있음)
				console.log(error);
			}
		});
	});
